...
static HashMemoryChunk ExecParallelHashPopChunkQueue(HashJoinTable table,
                                                     dsa_pointer *shared);
static bool ExecParallelHashTuplePrealloc(HashJoinTable hashtable, int batchno,
                                          size_t size);
...
MultiExecParallelHash(HashState *node)
{
    ...
    case PHJ_BUILD_HASHING_INNER:
         ...
         * below.
         */
        if (PHJ_GROW_BATCHES_PHASE(BarrierAttach(
                &pstate->grow_batches_barrier)) != PHJ_GROW_BATCHES_ELECTING)
            ExecParallelHashIncreaseNumBatches(hashtable);
        if (PHJ_GROW_BUCKETS_PHASE(BarrierAttach(
                &pstate->grow_buckets_barrier)) != PHJ_GROW_BUCKETS_ELECTING)
                    ...
                    bool keepNulls)
{
    ...
    ExecChooseHashTableSize(
        rows, outerNode->plan_width, OidIsValid(node->skewTable),
        state->parallel_state != NULL,
        state->parallel_state != NULL ? state->parallel_state->nparticipants - 1
                                      : 0,
    ...
    hashtable->hashCxt = AllocSetContextCreate(
        CurrentMemoryContext, "HashTableContext", ALLOCSET_DEFAULT_SIZES);

    hashtable->batchCxt = AllocSetContextCreate(
        hashtable->hashCxt, "HashBatchContext", ALLOCSET_DEFAULT_SIZES);
    ...
    if (nbatch > 1 && hashtable->parallel_state == NULL)
    {
        ...
        hashtable->innerBatchFile =
            (BufFile **)palloc0(nbatch * sizeof(BufFile *));
...
ExecChooseHashTableSize(double ntuples, int tupwidth, bool useskew,
                        bool try_combined_work_mem, int parallel_workers,
                        size_t *space_allowed, int *numbuckets, int *numbatches,
                        int *num_skew_mcvs)
{
    int tupsize;
    ...
    if (inner_rel_bytes + bucket_bytes > hash_table_bytes)
    {
        ...
        if (try_combined_work_mem)
        {
            ExecChooseHashTableSize(ntuples, tupwidth, useskew, false,
                                    parallel_workers, space_allowed, numbuckets,
                                    numbatches, num_skew_mcvs);
            return;
        }
...
ExecHashIncreaseNumBatches(HashJoinTable hashtable)
{
    ...
    while (oldchunks != NULL)
    {
        ...
        while (idx < oldchunks->used)
        {
            HashJoinTuple hashTuple =
                (HashJoinTuple)(HASH_CHUNK_DATA(oldchunks) + idx);
    ...
    printf("Hashjoin %p: freed %ld of %ld tuples, space now %zu\n", hashtable,
           nfreed, ninmemory, hashtable->spaceUsed);
#endif
    ...
    case PHJ_GROW_BATCHES_ELECTING:
                                 ...
                                 WAIT_EVENT_HASH_GROW_BATCHES_ELECTING))
        {
            ...
            if (pstate->old_nbatch == 1)
            {
                ...
                dbuckets =
                    Min(dbuckets, MaxAllocSize / sizeof(dsa_pointer_atomic));
                new_nbuckets = (int)dbuckets;
                new_nbuckets = Max(new_nbuckets, 1024);
                new_nbuckets = 1 << my_log2(new_nbuckets);
                dsa_free(hashtable->area, old_batch0->buckets);
                hashtable->batches[0].shared->buckets = dsa_allocate(
                    hashtable->area, sizeof(dsa_pointer_atomic) * new_nbuckets);
                buckets = (dsa_pointer_atomic *)dsa_get_address(
                    hashtable->area, hashtable->batches[0].shared->buckets);
            ...
            else
            {
                ...
                buckets = (dsa_pointer_atomic *)dsa_get_address(
                    hashtable->area, old_batch0->buckets);
                                 ...
                                 WAIT_EVENT_HASH_GROW_BATCHES_DECIDING))
        {
            ...
            for (i = 0; i < hashtable->nbatch; ++i)
            {
                    ...
                    batch->estimated_size > pstate->space_allowed)
                {
                    ...
                    if (batch->ntuples ==
                        hashtable->batches[parent].shared->old_ntuples)
...
ExecParallelHashRepartitionFirst(HashJoinTable hashtable)
{
    ...
    while ((chunk = ExecParallelHashPopChunkQueue(hashtable, &chunk_shared)))
    {
        ...
        {
            HashJoinTuple hashTuple =
                (HashJoinTuple)(HASH_CHUNK_DATA(chunk) + idx);
            MinimalTuple tuple = HJTUPLE_MINTUPLE(hashTuple);
            HashJoinTuple copyTuple;
            dsa_pointer shared;
            int bucketno;
...
ExecParallelHashRepartitionRest(HashJoinTable hashtable)
{
    ...
    old_batches = (ParallelHashJoinBatch *)dsa_get_address(hashtable->area,
                                                           pstate->old_batches);
    ...
    for (i = 1; i < old_nbatch; ++i)
    {
        ...
        old_inner_tuples[i] =
            sts_attach(ParallelHashJoinBatchInner(shared),
                       ParallelWorkerNumber + 1, &pstate->fileset);
    }
    ...
    for (i = 1; i < old_nbatch; ++i)
    {
        ...
        while (
            (tuple = sts_parallel_scan_next(old_inner_tuples[i], &hashvalue)))
        {
            size_t tuple_size = MAXALIGN(HJTUPLE_OVERHEAD + tuple->t_len);
            int bucketno;
            int batchno;
            ...
            ntuples > (hashtable->nbuckets_optimal * NTUP_PER_BUCKET))
        {
            ...
            if (hashtable->nbuckets_optimal <= INT_MAX / 2 &&
                hashtable->nbuckets_optimal * 2 <=
                    MaxAllocSize / sizeof(HashJoinTuple))
            {
                hashtable->nbuckets_optimal *= 2;
...
retry:
    ...
    if (batchno == 0)
    {
        ...
        hashTuple = ExecParallelHashTupleAlloc(
            hashtable, HJTUPLE_OVERHEAD + tuple->t_len, &shared);
        if (hashTuple == NULL)
            goto retry;
                                        ...
                                        TupleTableSlot *slot, uint32 hashvalue)
{
    ...
    memcpy(HJTUPLE_MINTUPLE(hashTuple), tuple, tuple->t_len);
    HeapTupleHeaderClearMatch(HJTUPLE_MINTUPLE(hashTuple));
    ExecParallelHashPushTuple(&hashtable->buckets.shared[bucketno], hashTuple,
                              shared);
 ...
 */
bool
ExecHashGetHashValue(HashJoinTable hashtable, ExprContext *econtext,
    ...
    foreach (hk, hashkeys)
    {
        ...
        else
        {
            ...
            hkey = DatumGetUInt32(FunctionCall1Coll(
                &hashfunctions[i], hashtable->collations[i], keyval));
            hashkey ^= hkey;
        }
...
ExecScanHashBucket(HashJoinState *hjstate, ExprContext *econtext)
{
    ...
    {
        //elog(NOTICE, "not find tuple, next");
        if (hashTuple->hashvalue == hashvalue)
        {
            //elog(NOTICE,"find tuple");
            if (hjstate->scanBucket)
            {
...
ExecParallelScanHashBucket(HashJoinState *hjstate, ExprContext *econtext)
{
    ...
    if (hashTuple != NULL)
        ...
        hashTuple =
            ExecParallelHashFirstTuple(hashtable, hjstate->hj_CurBucketNo);
...
ExecHashBuildSkewHash(HashJoinTable hashtable, Hash *node, int mcvsToUse)
{
     ...
     */
    statsTuple = SearchSysCache3(
        STATRELATTINH, ObjectIdGetDatum(node->skewTable),
        Int16GetDatum(node->skewColumn), BoolGetDatum(node->skewInherit));
                         ...
                         ATTSTATSSLOT_VALUES | ATTSTATSSLOT_NUMBERS))
    {
        ...
        for (i = 0; i < mcvsToUse; i++)
        {
            ...
            hashvalue = DatumGetUInt32(FunctionCall1Coll(
                &hashfunctions[0], hashtable->collations[0], sslot.values[i]));
            ...
            hashtable->skewBucket[bucket] =
                (HashSkewBucket *)MemoryContextAlloc(hashtable->batchCxt,
...
ExecHashSkewTableInsert(HashJoinTable hashtable, TupleTableSlot *slot,
                        uint32 hashvalue, int bucketNumber)
{
    bool shouldFree;
    MinimalTuple tuple = ExecFetchSlotMinimalTuple(slot, &shouldFree);
    HashJoinTuple hashTuple;
...
ExecHashInitializeWorker(HashState *node, ParallelWorkerContext *pwcxt)
{
    ...
    shared_info = (SharedHashInfo *)shm_toc_lookup(
        pwcxt->toc, node->ps.plan->plan_node_id, false);
...
dense_alloc(HashJoinTable hashtable, Size size)
{
    ...
    if (size > HASH_CHUNK_THRESHOLD)
    {
        ...
        newChunk = (HashMemoryChunk)MemoryContextAlloc(
            hashtable->batchCxt, HASH_CHUNK_HEADER_SIZE + size);
        ...
        (hashtable->chunks->maxlen - hashtable->chunks->used) < size)
    {
        /* allocate new chunk and put it at the beginning of the list */
        newChunk = (HashMemoryChunk)MemoryContextAlloc(
            hashtable->batchCxt, HASH_CHUNK_HEADER_SIZE + HASH_CHUNK_SIZE);
                           ...
                           dsa_pointer *shared)
{
    ...
    if (pstate->growth != PHJ_GROWTH_DISABLED)
    {
        ...
        if (hashtable->batches[0].at_least_one_chunk &&
            hashtable->batches[0].shared->size + chunk_size >
...
ExecParallelHashJoinSetUpBatches(HashJoinTable hashtable, int nbatch)
{
    ...
    hashtable->nbatch = nbatch;
    hashtable->batches = (ParallelHashJoinBatchAccessor *)palloc0(
        sizeof(ParallelHashJoinBatchAccessor) * hashtable->nbatch);
    ...
    for (i = 0; i < hashtable->nbatch; ++i)
    {
        ...
        accessor->inner_tuples = sts_initialize(
            ParallelHashJoinBatchInner(shared), pstate->nparticipants,
            ParallelWorkerNumber + 1, sizeof(uint32),
            SHARED_TUPLESTORE_SINGLE_PASS, &pstate->fileset, name);
        ...
        accessor->outer_tuples = sts_initialize(
            ...
            pstate->nparticipants, ParallelWorkerNumber + 1, sizeof(uint32),
            SHARED_TUPLESTORE_SINGLE_PASS, &pstate->fileset, name);
    }
...
ExecParallelHashEnsureBatchAccessors(HashJoinTable hashtable)
{
    ...
    batches = (ParallelHashJoinBatch *)dsa_get_address(hashtable->area,
                                                       pstate->batches);
...
ExecParallelHashTableAlloc(HashJoinTable hashtable, int batchno)
{
    ...
    buckets =
        (dsa_pointer_atomic *)dsa_get_address(hashtable->area, batch->buckets);
                          ...
                          dsa_pointer tuple_shared)
{
    for (;;)
    {
        ...
        if (dsa_pointer_atomic_compare_exchange(head, &tuple->next.shared,
                                                tuple_shared))
            break;
    }
}
...
ExecParallelHashTableSetCurrentBatch(HashJoinTable hashtable, int batchno)
{
    ...
    hashtable->buckets.shared = (dsa_pointer_atomic *)dsa_get_address(
        hashtable->area, hashtable->batches[batchno].shared->buckets);
